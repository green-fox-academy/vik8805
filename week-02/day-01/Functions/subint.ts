...
function findMatchingIndexes(number: number, listOfNumbers: number[]): number[] {
  let indicesOfNumbers: number[] = [];
  listOfNumbers.forEach(function (value: number, index: number) {
    if (value.toLocaleString().indexOf(number.toLocaleString()) !== -1) {
      indicesOfNumbers.push(index);
    }
  });
  return indicesOfNumbers;
}
