...
for (let g: number = 0; g < numbOfQuarters; g++) {
  for (let h: number = 0; h < numbOfQuarters; h++) {
    for (let i: number = 0; i < numbOfLines; i++) {
      ...
      drawLinePlay(newCanvasWidth / numbOfLines * i + newCanvasWidth * h, newCanvasHeight + newCanvasHeight * g, newCanvasWidth * h, newCanvasHeight / numbOfLines * i + newCanvasHeight * g);
    };
    for (let j: number = 0; j < numbOfLines; j++) {
      ctx.strokeStyle = '#b354f0';
      drawLinePlay(newCanvasWidth / numbOfLines * j + newCanvasWidth * h, newCanvasHeight * g, newCanvasWidth + newCanvasWidth * h, newCanvasHeight / numbOfLines * j + newCanvasHeight * g);
    };
  };
};